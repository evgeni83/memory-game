...
class App extends React.Component  {
    ...
    stopTheGame() {
        ...
        this.setState({
            results: [this.state.timer, ...this.state.results]
        });
    ...
    saveResultsToLocalStorage() {
        const resultsJson = JSON.stringify(this.state.results);
    ...
    componentDidMount() {
        if (localStorage.hasOwnProperty("results")) {
            ...
            this.setState({results: JSON.parse(resultsJson)})
        }
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        if (this.state.localTimer >= 5 && this.state.cardsToCompare.length < 2) {
            this.closeCards()
        }
    }
        ...
        return (
            <div className="App">
                <header className="App-header">
                    <h1>MEMORY GAME</h1>
                    <span className="timer">Timer <CommonTimer timer={ this.state.timer }/></span>
                </header>
                <main>
                    <StartScreen
